...
packages <- c("tidyverse", "haven", "table1", "purrr", "MASS", "randomForest", "doParallel", "mice")
for (pkg in packages) {
...
# Prepare the dataset
# Using demographic and dietary variables to predict blood pressure (sybp and dibp)
features <- cleaned_data_imputed %>% 
  dplyr::select(
    -c(SEQN, sybp, dibp) # Remove unique ID and target variables
  )
...
# Evaluate the model
predicted_values <- predict(rf_model, test_data)
actual_values <- test_data$sybp
